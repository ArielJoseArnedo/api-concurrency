...
public class FutureEither<L, R> {
    ...
    public static <L, R> FutureEither<L, R> of(Future<Either<L, R>> value) { return new FutureEither<>(value); }

    public static <L, R> FutureEither<L, R> fromEither(Either<L, R> either) { return new FutureEither<>(Future.successful(either)); }

    public static <L, R> FutureEither<L, R> right(R r) { return new FutureEither<>(Future.successful(Either.right(r))); }

    public static <L, R> FutureEither<L, R> left(L l) { return new FutureEither<>(Future.successful(Either.left(l))); }
    ...
    public <U> FutureEither<L, U> flatMap(Function1<R, FutureEither<L, U>> f) {
        return new FutureEither<>(
          this.value.flatMap(
            e -> e.fold(l -> Future.successful(Either.left(l)), r -> f.apply(r).getValue())));
    }
